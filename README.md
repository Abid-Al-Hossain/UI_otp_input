# UI Foundry OTP Input Studio

Standalone visual OTP Input generator built with Next.js, React, and TypeScript.

Standalone OTP Input component studio for building native, preview-honest, React-exportable UI patterns that match the UI Foundry product line.
...
- designers building OTP Input UI quickly
- developers who need editable React starting points
- marketplace buyers who want a self-contained OTP Input generator
...
The editor is focused on controls that are native to OTP Input and useful in real product work:
- digit count, mask mode, separator grouping, numeric keyboard hints, and auto-advance intent
- label, helper, error, success, disabled, required, and validation state
- cell size, gap, radius, border, focus, filled, invalid, and disabled styling
- paste behavior expectations and accessible per-digit labels
- React export for the current OTP state model

The goal is maximum useful depth without adding unrelated controls that would make the component stop reading as OTP Input.
...
This repo is intended to publish as Abid-Al-Hossain/UI_otp_input.
