...
const nativeFeatures = [
  "Field metadata: id, name, title, tabIndex, dir, lang, autocomplete where native",
  "Validation: required, disabled, readonly-safe states, helper, error, success, description",
  "Input affordances: placeholder, prefix/suffix, clear action, keyboard hints, min/max/step where native",
  "State preview: default, focus, disabled, invalid, filled, empty"
] as const;

const defaultState: StudioState = {
  title: "OTP / PIN Input Studio",
  description: "Native otp / pin input pattern with preview/export-honest controls.",
  label: "OTP / PIN Input",
  value: "otp input",
  helper: "Production-ready, accessible, React-only output.",
  id: "otp-input-instance",
  name: "otp-input",
  required: false,
  disabled: false,
  interactive: true,
...
const presets: Preset[] = [
  { family: "forms", archetype: "SaaS", variant: "glass", size: "balanced", tags: ["premium", "dark", "export-safe"], state: defaultState },
  { family: "forms", archetype: "Editorial", variant: "soft", size: "spacious", tags: ["marketplace", "content"], state: { ...defaultState, density: "spacious", background: "#f8fafc", foreground: "#0f172a", border: "#cbd5e1", accent: "#f59e0b", shadow: 10 } },
  { family: "forms", archetype: "Admin", variant: "crisp", size: "compact", tags: ["dashboard", "dense"], state: { ...defaultState, density: "compact", radius: 10, width: 320, height: 140, gap: 8, fontSize: 14, accent: "#22c55e" } },
  { family: "forms", archetype: "Commerce", variant: "hero", size: "spacious", tags: ["commerce", "bold"], state: { ...defaultState, title: "Conversion OTP / PIN Input", accent: "#fb7185", background: "#1e1b4b", width: 420, height: 220, shadow: 36 } },
];
    ...
    <main className="mx-auto grid min-h-screen max-w-[1500px] gap-6 px-5 py-6 lg:grid-cols-[430px_1fr]">
      <aside className="space-y-5">
        <header className="rounded-[2rem] border border-white/10 bg-slate-950/70 p-5 shadow-2xl shadow-black/30">
          ...
          <h1 className="mt-3 text-3xl font-black tracking-tight">OTP / PIN Input Studio</h1>
          <p className="mt-2 text-sm text-slate-400">Self-contained, React-only, buyer-ready otp / pin input generator.</p>
        </header>
